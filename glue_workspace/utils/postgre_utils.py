...
class PostgresHandler:
    def __init__(self, secret_manager_name="cedc/dags/postgres"):
        ...
        """
        self._pool = None
        sm_info = json.loads(AWSSecretsManager(region_name = 'ap-northeast-1').get_secret(secret_name=secret_manager_name))
    ...
    def init_pgsql_pool(self):
        ...
        try:
            ...
            raise ValueError('ERROR: create postgresql pool error caused by {0}'.format(str(e)))
    ...
    def get_record(self, sql):
        ...
        try:
            ...
            raise ValueError("get record is bad!")
        else:
            self._cur.close()
            # self._conn.close()
            return dict_results
    ...
    def execute_insert(self, run_id=None, job_name=None, status=None):
        ...
        try:
            ...
            raise ValueError("execute_insert is bad!")
        else:
            self._cur.close()
            # self._conn.close()
            return flag
    ...
    def execute_update(self, run_id=None, job_name=None, status=None):
        ...
        try:
            ...
            self._conn.rollback()
            logger.error("执行失败, %s" % err)
            raise ValueError("execute_update is bad!")
        else:
            self._cur.close()
            # self._conn.close()
            return flag
    ...
    def execute_delete(self, run_id=None):
        ...
        try:
            ...
            self._conn.rollback()
            logger.error("执行失败, %s" % err)
            raise ValueError("execute_delete is bad!")
        else:
            self._cur.close()
            # self._conn.close()
            return flag

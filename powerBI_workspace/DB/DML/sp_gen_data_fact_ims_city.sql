CREATE PROCEDURE sp_gen_data_fact_ims_city(IN cnt INT)
BEGIN
...
WHILE i <= cnt DO
	...
	SET V_CITYCODE=(SELECT CityCode FROM dim_report_line ORDER BY RAND() LIMIT 1);
...
SET V_PCKAGE=(SELECT CASE FLOOR(RAND() * 6)
    ...
    INSERT INTO `fact_ims_city` (`YM`, `CityCode`, `BrandCode`,`Corp`, `Product`, `Molecule`,`IMSCITY_Amount`, `IMSCITY_QTY` , `Defined_MKT`, `FocusCompeGrp`, `Manufactory`,`Flag_Aspen`, `PACKAGE`, `PACK_SIZE`, `NFCI_Code`, `NFCI_Description`,`etl_timestamp`)
	VALUES (V_YM, 
	        V_CITYCODE,
			V_BRD,
			CONCAT('Corp', i), 

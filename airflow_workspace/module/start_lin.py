...
class Start:
    ...
    def run(self,event):
        ...
        task_name = self.query_fact_task_insert(dag_name=self.dag_name,task_name=self.task_name,task_status=Constants.GLUE_RUNNING)
    ...
    def query_fact_task_insert(self,dag_name,task_name,task_status):
        # 执行insert_fact_task
        insert_sql_fact_task = """ insert into fact_task_details 
                                     ...
                                     """.format(dag_name=dag_name,task_name=task_name, task_status=task_status)
...
#     event= {"dag_name":"start_lin","task_name": "task_cedc_sales_prelanding_push_params"}
#     Start().run(event)
#
